...
def runtime_probe(root, framework, routes, entrypoint="main:app", mutate=False):
    if framework not in {"fastapi", "flask"}:
        return [], SensorResult(name="runtime", status="SKIPPED", detail="Only Flask/FastAPI runtime supported")
    ...
    try:
        run = run_container(image, ["python", "/opt/traceguard/probe.py"], root,
            extra={"request.json": json.dumps(config).encode()}, output_file="/workspace/out/runtime.json", timeout=180)
        ...
        return findings, SensorResult(name="runtime", status="COMPLETED", finding_count=len(findings),
            detail=f"{len(observations)} bounded observations in networkless Docker sandbox", image=run.image,
